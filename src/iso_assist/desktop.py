...
from iso_assist.config import DOCS_PATH
from iso_assist.models.doc_memory import (
    attach_document,
    get_documents,
...
from iso_assist.services.ollama_manager import is_model_available, pull_model_now
from iso_assist.services.rag_engine import get_kb_stats, retrieve, ask_stream

if getattr(sys, "frozen", False):
    # In a PyInstaller bundle __file__ is inside the read-only _MEIPASS temp dir.
    UI_HTML = Path(sys._MEIPASS) / "iso_assist" / "ui" / "index.html"  # type: ignore[attr-defined]
else:
...
class Api:
    ...
    def get_ollama_status(self) -> dict:
        try:
            import ollama
            from iso_assist.config import OLLAMA_BASE_URL
...
def _seed_bundled_kb():
    ...
    if not getattr(sys, "frozen", False):
        return
    from iso_assist.config import ROOT, BUNDLE_DIR, CHROMA_PATH
